...
$(document).ready(function(){
	...
	if($('#morris_extra_line_chart_1').length > 0) {
		var cty_data = cty_orders;
		// var cty_dataNew = cty_money;
		var lineChart = Morris.Line({
        element: 'morris_extra_line_chart_1',
        data: cty_data ,
        xkey: '日期',
        ykeys: cty_num_lable,
        labels: cty_num_lable,
        pointSize: 2,
        fillOpacity: 0,
		lineWidth:2,
	...
	var swichMorris = function() {
		if($("#morris_switch").is(":checked")) {
			// lineChart.setData(cty_data);
			// lineChart.redraw();
		} else {
			// lineChart.setData(cty_dataNew);
			// lineChart.redraw();
		}
	}
	swichMorris();	
